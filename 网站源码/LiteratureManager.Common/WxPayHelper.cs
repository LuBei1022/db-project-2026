...
namespace LiteratureManager.Common
{
    public class WxPayHelper
    {
        ...
        /// <returns></returns>
        public async Task<returnParameters>  NativeGetprepay(string description, int price, string out_trade_no, string notify_url, string payer_client_ip, string attach = "", string currency = "CNY")
        {
            ...
            var req = new GenerateOrderModelForWxPay
            {
                ...
                {
                    total = Convert.ToInt32(price * 100),//金额
                    currency = currency
                },
                scene_info = new WxPaySceneInfo
                {
            ...
            ImportDataLog.WriteLog(LogType.Error, "respStr：" + respStr);
            // 如果下单成功，就解析返回的结果，把prepay_id解析出来
            var returnParameters = respStr.ToObject<returnParameters>();
        ...
        {
            return new HttpClient(new WxPayRequestHandler(_mchid, _serialNo, _privateKey))
            {
                Timeout = TimeSpan.FromMilliseconds(UploadPolicy.ExternalRequestTimeoutMs)
            };
        }
    }
}
