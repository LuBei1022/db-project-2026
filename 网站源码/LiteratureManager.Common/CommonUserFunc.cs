...
namespace LiteratureManager.Common
{
    public class CommonUserFunc
    {
        ...
        public static BLLBase<ResourceCopy_list> ResourceCopy_listbll = new BLLBase<ResourceCopy_list>();
        public static BLLBase<ResourceDataInfoCopy_list> ResourceDataInfoCopy_listbll = new BLLBase<ResourceDataInfoCopy_list>();
        public static BLLBase<ResourceDataCopy_list> ResourceDataCopy_listbll = new BLLBase<ResourceDataCopy_list>();
        public static BLLBase<ResourceDataImgCopy_list> ResourceDataImgCopy_listbll = new BLLBase<ResourceDataImgCopy_list>();
        public static BLLBase<ResourceDataInfoDataCopy_list> ResourceDataInfoDataCopy_listbll = new BLLBase<ResourceDataInfoDataCopy_list>();
        ...
        public static BLLBase<ResourceData_list> ResourceData_listbll = new BLLBase<ResourceData_list>();
        public static BLLBase<ResourceDownloadLog_List> ResourceDownloadLog_Listbll = new BLLBase<ResourceDownloadLog_List>();
        public static BLLBase<GeneratedAssetRecord_list> GeneratedAssetRecord_listbll = new BLLBase<GeneratedAssetRecord_list>();
        ...
        public static int GetResourceCommentReplyNum(int user_id)
        {
            int num = 0;
            DataTable ResourceCommentReply_list_dt = ResourceCommentReply_listbll.GetDatatable("select count(1) as num from ResourceCommentReply_list where msguser_id=" + user_id + " and status=1");
            if (ResourceCommentReply_list_dt != null && ResourceCommentReply_list_dt.Rows.Count > 0)
            {
                num = Function.ConvertTo<int>(ResourceCommentReply_list_dt.Rows[0]["num"].ToString(), 0);
            }
            ResourceCommentReply_list_dt.Dispose();
            return num;
        }
        public static int GetResourceDataInfoDataSumFunc(string ResourceDataInfoCopy_id)
        {
            int num = 0;
            DataTable ResourceDataInfoDataCopy_list_dt = ResourceDataInfoDataCopy_listbll.GetDatatable("select sum(weight) as num from ResourceDataInfoDataCopy_list where ResourceDataInfoCopy_id=" + Function.ConvertTo<long>(ResourceDataInfoCopy_id, 0));
            if (ResourceDataInfoDataCopy_list_dt != null && ResourceDataInfoDataCopy_list_dt.Rows.Count > 0)
            {
                num = Function.ConvertTo<int>(ResourceDataInfoDataCopy_list_dt.Rows[0]["num"].ToString(), 0);
            }
            ResourceDataInfoDataCopy_list_dt.Dispose();
            return num;
        }
        ...
        public static string GetDataTemporaryAddFunc(string name, decimal time, decimal diameter, string[] ConsumableMaterialArr, string[] ColorArr, string[] WeightArr, string upload_pic_cover, int userid)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                {
                    bool isyes = true;
                    if (string.IsNullOrWhiteSpace(name) || time <= 0 || diameter <= 0)
                    {
                        isyes = false;
                    }
                    string sql = string.Empty;
                    if (!string.IsNullOrWhiteSpace(upload_pic_cover))
                    {
                        upload_pic_cover = upload_pic_cover.Replace("/A_UpLoad/upload_pic/", "");
                        if (CommonFunc.GetImgBool(upload_pic_cover))
                        {
                            sql = "DELETE FROM userimg_list WHERE upload_pic_img='" + upload_pic_cover + "' and userid=" + user_list.id;
                        }
                        else
                        {
                            isyes = false;
                    ...
                    ArrayList dic_data = new ArrayList();
                    if (ConsumableMaterialArr != null && ConsumableMaterialArr.Length > 0 && ColorArr != null && WeightArr != null && ConsumableMaterialArr.Length == ColorArr.Length && ConsumableMaterialArr.Length == WeightArr.Length)
                    {
                        foreach (string item in ConsumableMaterialArr)
                        {
                        ...
                        foreach (string item in ColorArr)
                        {
                            if (!string.IsNullOrWhiteSpace(item))
                            {
                        ...
                        int index_ = 0;
                        foreach (string item in WeightArr)
                        {
                            decimal item_Weight = Function.ConvertTo<decimal>(item, 0);
                            if (item_Weight > 0)
                            {
                                sql += "ξLiteratureManagerξINSERT INTO ResourceDataInfoDataCopy_list (ResourceDataInfoCopy_id, weight, ColorVal, ConsumableMaterialVal) VALUES (LiteratureManagerteshu, " + item_Weight + ", N'" + ColorArr[index_] + "', N'" + ConsumableMaterialArr[index_] + "')";
                                sum_Weight += item_Weight;
                                Dictionary<string, object> drow_ = new Dictionary<string, object>();
                                drow_.Add("weight", item_Weight);
                                drow_.Add("Color", ColorArr[index_]);
                                drow_.Add("ConsumableMaterial", ConsumableMaterialArr[index_]);
                                dic_data.Add(drow_);
                            }
                            else
                            {
                    ...
                    if (isyes)
                    {
                        ...
                        ResourceDataInfoCopy_list ResourceDataInfoCopy_list = new ResourceDataInfoCopy_list();
                        ResourceDataInfoCopy_list.name = Function.HtmlEncode(name);
                        ResourceDataInfoCopy_list.upload_pic_cover = upload_pic_cover;
                        ResourceDataInfoCopy_list.time_h = time;
                        ResourceDataInfoCopy_list.diameter = diameter;
                        ResourceDataInfoCopy_list.addtime = DateTime.Now;
                        ResourceDataInfoCopy_list.user_id = userid;
                        ResourceDataInfoCopy_list.status = 0;

                        StringBuilder strSql = new StringBuilder();
                        strSql.Append("insert into ResourceDataInfoCopy_list(");
                        strSql.Append("name, upload_pic_cover,time_h, diameter, addtime, status, user_id)");
                        strSql.Append(" values (");
                        strSql.Append(" @name, @upload_pic_cover,@time_h, @diameter, @addtime, @status, @user_id)");
                        strSql.Append(";select @@IDENTITY");
                        SqlParameter[] parameters = {
                        new SqlParameter("@name", SqlDbType.NVarChar,500),
                          new SqlParameter("@upload_pic_cover",SqlDbType.NVarChar,250),
                          new SqlParameter("@time_h",SqlDbType.Decimal,1),
                          new SqlParameter("@diameter",SqlDbType.Decimal,2),
                          new SqlParameter("@addtime",SqlDbType.DateTime),
                          new SqlParameter("@status",SqlDbType.Int),
                          new SqlParameter("@user_id",SqlDbType.Int)
                                    };

                        parameters[0].Value = ResourceDataInfoCopy_list.name;
                        parameters[1].Value = ResourceDataInfoCopy_list.upload_pic_cover;
                        parameters[2].Value = ResourceDataInfoCopy_list.time_h;
                        parameters[3].Value = ResourceDataInfoCopy_list.diameter;
                        parameters[4].Value = ResourceDataInfoCopy_list.addtime;
                        parameters[5].Value = ResourceDataInfoCopy_list.status;
                        parameters[6].Value = ResourceDataInfoCopy_list.user_id;

                        int addid = ResourceDataInfoCopy_listbll.Add_R_Id_(parameters, strSql, sql);
                        if (addid > 0)
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "保存成功！", ["addid"] = addid, ["name"] = Function.HtmlDiscode(ResourceDataInfoCopy_list.name), ["upload_pic_cover"] = ResourceDataInfoCopy_list.upload_pic_cover, ["time"] = ResourceDataInfoCopy_list.time_h, ["diameter"] = ResourceDataInfoCopy_list.diameter, ["sum_Weight"] = sum_Weight, ["dic_data"] = dic_data });
                        }
                        else
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "保存失败，请稍后再试！" });
                        }
                    }
                    else
                    {
                        str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "请求数据异常！" });
                    }
                }
                else
                {
        ...
        public static string GetDelResourceDataFunc(int ResourceDataCopy_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ResourceDataCopy_list ResourceDataCopy_list = ResourceDataCopy_listbll.SelectSingle("id=" + ResourceDataCopy_id);
                    if (ResourceDataCopy_list != null && ResourceDataCopy_list.id > 0)
                    {
                        ResourceCopy_list ResourceCopy_list = ResourceCopy_listbll.SelectSingle("id=" + ResourceDataCopy_list.ResourceCopy_id + " and userid=" + user_list.id + " and status in(-2,-1)");
                        if (ResourceCopy_list != null && ResourceCopy_list.userid > 0)
                        {
                            string sql = "DELETE FROM ResourceDataInfoDataCopy_list WHERE ResourceDataInfoCopy_id in(select id from ResourceDataInfoCopy_list WHERE ResourceDataCopy_id=" + ResourceDataCopy_list.id + ")";
                            sql += "ξLiteratureManagerξDELETE FROM ResourceDataInfoCopy_list WHERE ResourceDataCopy_id=" + ResourceDataCopy_list.id;
                            sql += "ξLiteratureManagerξDELETE FROM ResourceDataImgCopy_list WHERE ResourceDataCopy_Id=" + ResourceDataCopy_list.id;
                            sql += "ξLiteratureManagerξDELETE FROM ResourceDataCopy_list WHERE id=" + ResourceDataCopy_list.id;


                            DataTable ResourceDataImgCopy_listdt = ResourceDataImgCopy_listbll.GetDatatable("select upload_pic_info from ResourceDataImgCopy_list where ResourceDataCopy_id=" + ResourceDataCopy_list.id);
                            DataTable ResourceDataInfoCopy_listdt = ResourceDataInfoCopy_listbll.GetDatatable("select upload_pic_cover from ResourceDataInfoCopy_list where ResourceDataCopy_id=" + ResourceDataCopy_list.id);

                            if (ResourceDataCopy_listbll.Sql_D(sql))
                            {
                                try
                                {
                                    if (ResourceDataInfoCopy_listdt != null && ResourceDataInfoCopy_listdt.Rows.Count > 0)
                                    {
                                        foreach (DataRow item in ResourceDataInfoCopy_listdt.Rows)
                                        {
                                ...
                                try
                                {
                                    if (ResourceDataImgCopy_listdt != null && ResourceDataImgCopy_listdt.Rows.Count > 0)
                                    {
                                        foreach (DataRow item in ResourceDataImgCopy_listdt.Rows)
                                        {
                                            Function.FileDelete("/A_UpLoad/upload_pic/" + item["upload_pic_info"].ToString());
                                        }
                                    }
                                ...
                                try
                                {
                                    if (!string.IsNullOrWhiteSpace(ResourceDataCopy_list.upload_pic_cover))
                                    {
                                        Function.FileDelete("/A_UpLoad/upload_pic/" + ResourceDataCopy_list.upload_pic_cover);
                                    }
                                ...
                                try
                                {
                                    if (!string.IsNullOrWhiteSpace(ResourceDataCopy_list.up_filename_stl))
                                    {
                                        CommonFunc.DeleteTrackedUploadRecord(ResourceDataCopy_list.up_filename_stl);
                                        Function.FileDelete("/A_UpLoad/upload_file/" + ResourceDataCopy_list.up_filename_stl);
                                    }
                                ...
                                try
                                {
                                    if (!string.IsNullOrWhiteSpace(ResourceDataCopy_list.up_filename_3mf))
                                    {
                                        CommonFunc.DeleteTrackedUploadRecord(ResourceDataCopy_list.up_filename_3mf);
                                        Function.FileDelete("/A_UpLoad/upload_file/" + ResourceDataCopy_list.up_filename_3mf);
                                    }
                                ...
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "删除版本记录成功！" });
                            }
                            else
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "删除版本记录失败！" });
                            }
                        }
                        else
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "删除数据记录异常！" });
                        }
                    }
                    else
                    {
        ...
        [Obsolete("旧 AI/3D GeneratedAsset 业务入口，最终交付版不再暴露到主流程。")]
        public static string GetGeneratedAssetDeleteFunc(long task_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    GeneratedAssetRecord_list GeneratedAssetRecord_list = GeneratedAssetRecord_listbll.SelectSingle("id=" + task_id+ " and userid="+ user_list.id);
                    if (GeneratedAssetRecord_list != null && GeneratedAssetRecord_list.id > 0)
                    {
                        string sql = "DELETE FROM GeneratedAssetRecord_list WHERE id=" + GeneratedAssetRecord_list.id;
                        if (ResourceDataCopy_listbll.Sql_D(sql))
                        {
                            if (!string.IsNullOrWhiteSpace(GeneratedAssetRecord_list.cos_name))
                            {
                                try
                                {
                                    PutObjectModel.DeleteObject("/literature/assets/" + GeneratedAssetRecord_list.cos_name + "/" + GeneratedAssetRecord_list.cos_name + ".png");
                                    PutObjectModel.DeleteObject("/literature/assets/" + GeneratedAssetRecord_list.cos_name + "/" + GeneratedAssetRecord_list.cos_name + ".zip");
                                }
                                catch (Exception)
                                {
        ...
        public static int GetResourceCollectNumFunc()
        {
            int num = 0;
            user_list user_list = GetUserLoginStatus();
            if (user_list != null && user_list.id > 0)
            {
                DataTable ResourceCollect_List_dt = ResourceCollect_Listbll.GetDatatable("select count(1) as num from ResourceCollect_List where user_id=" + user_list.id);
                if (ResourceCollect_List_dt != null && ResourceCollect_List_dt.Rows.Count > 0)
                {
                    num = Function.ConvertTo<int>(ResourceCollect_List_dt.Rows[0]["num"].ToString(), 0);
                }
                ResourceCollect_List_dt.Dispose();
            }
            return num;
        }
        public static string GetResourceLikeAddFunc(int Resource_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    Resource_list Resource_list = Resource_listbll.SelectSingle("id=" + Resource_id);
                    if (Resource_list != null && Resource_list.id > 0)
                    {
                        ResourceLike_List ResourceLike_List = ResourceLike_Listbll.SelectSingle("resource_id=" + Resource_id + " and user_id=" + user_list.id);
                        if (ResourceLike_List != null && ResourceLike_List.resource_id > 0)
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "已点赞！", ["isCollect"] = true, ["num_dianzan"] = Resource_list.num_dianzan });
                            //取消点赞
                            //string sql = "DELETE FROM ResourceLike_List WHERE resource_id=" + Resource_list.id + " and user_id=" + user_list.id;
                            //sql += "ξLiteratureManagerξUPDATE Resource_list SET num_shoucang=num_shoucang-1 WHERE id=" + Resource_list.id;
                            //if (ResourceLike_Listbll.Sql_D(sql))
                            //{
                            //    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "取消点赞成功！", ["isCollect"] = false });
                            //}
                            //else
                            //{
                            //    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "取消点赞失败，请稍后再试！" });
                            //}
                        }
                        else
                        {
                            //点赞
                            string sql = "INSERT INTO ResourceLike_List (resource_id,user_id,addtime,resource_user_id) VALUES (" + Resource_list.id + "," + user_list.id + ",GETDATE()," + Resource_list.userid + ")";
                            sql += "ξLiteratureManagerξUPDATE Resource_list SET num_dianzan=num_dianzan+1 WHERE id=" + Resource_list.id;
                            if (ResourceLike_Listbll.Sql_D(sql))
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "点赞成功！", ["isCollect"] = true, ["num_dianzan"] = Resource_list.num_dianzan + 1 });
                            }
                            else
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "点赞失败，请稍后再试！" });
                            }
                        }
                    }
                    else
                    {
                        str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "操作参数异常，请稍后再试！" });
                    }
                }
                else
                {
        ...
        public static string GetResourceCollectFunc(int Resource_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ...
                    if (Resource_list != null && Resource_list.id > 0)
                    {
                        ResourceCollect_List ResourceCollect_List = ResourceCollect_Listbll.SelectSingle("resource_id=" + Resource_id + " and user_id=" + user_list.id);
                        if (ResourceCollect_List != null && ResourceCollect_List.resource_id > 0)
                        {
                            //取消收藏
                            string sql = "DELETE FROM ResourceCollect_List WHERE resource_id=" + Resource_list.id + " and user_id=" + user_list.id;
                            sql += "ξLiteratureManagerξUPDATE Resource_list SET num_shoucang=num_shoucang-1 WHERE id=" + Resource_list.id;
                            if (ResourceCollect_Listbll.Sql_D(sql))
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "取消收藏成功！", ["isCollect"] = false, ["num_shoucang"] = Resource_list.num_shoucang - 1 });
                            }
                            else
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "取消收藏失败，请稍后再试！" });
                            }
                        }
                        else
                        {
                            //收藏
                            string sql = "INSERT INTO ResourceCollect_List (resource_id,user_id,addtime,resource_user_id) VALUES (" + Resource_list.id + "," + user_list.id + ",GETDATE()," + Resource_list.userid + ")";
                            sql += "ξLiteratureManagerξUPDATE Resource_list SET num_shoucang=num_shoucang+1 WHERE id=" + Resource_list.id;
                            if (ResourceCollect_Listbll.Sql_D(sql))
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "收藏成功！", ["isCollect"] = true, ["num_shoucang"] = Resource_list.num_shoucang + 1 });
                            }
                            else
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "收藏失败，请稍后再试！" });
                            }
                        }
                    }
                    else
        ...
        public static string GetResourceFormatShowFunc(int Resource_id, string isshow)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    Resource_list Resource_list = Resource_listbll.SelectSingle("id=" + Resource_id + " and userid=" + user_list.id);
                    if (Resource_list != null && Resource_list.id > 0)
                    {
                        if (isshow == "1")
                        {
                        ...
                        //修改公开状态
                        if (Resource_listbll.Update("isshow=" + isshow, "id=" + Resource_list.id))
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "修改公开状态成功！" });
                        }
                        else
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "修改公开状态失败，请稍后再试！" });
                        }
                    }
                    else
                    {
        ...
        [Obsolete("旧 AI/3D 模型展示业务入口，最终交付版不再暴露到主流程。")]
        public static string Get3DAiModelShowFunc(int GeneratedAssetRecord_id, string isshow)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    GeneratedAssetRecord_list GeneratedAssetRecord_list = GeneratedAssetRecord_listbll.SelectSingle(" id=" + GeneratedAssetRecord_id+ " and userid="+ user_list.id);
                    if (GeneratedAssetRecord_list != null && GeneratedAssetRecord_list.id > 0)
                    {
                        if (isshow == "1")
                        {
                        ...
                        //修改公开状态
                        if (GeneratedAssetRecord_listbll.Update("isshow="+ isshow, "id=" + GeneratedAssetRecord_list.id))
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "修改公开状态成功！"});
                        }
                        else
        ...
        [Obsolete("旧 Resource 评论入口，UserCommon.ashx.cs 已移除对应 btn 分发。")]
        public static string GetMsgAddFunc(long resource_id, string info, string[] ImgArr, string reply_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ...
                    {
                        Resource_list Resource_list = Resource_listbll.SelectSingle("id=" + resource_id + " and status=1");
                        if (Resource_list != null && Resource_list.id > 0)
                        {
                            int msguser_id = 0;
                            if (Function.ConvertTo<long>(reply_id, 0) > 0)
                            {
                                ResourceComment_list ResourceComment_list = ResourceComment_listbll.SelectSingle("id=" + reply_id + " and Resource_id=" + resource_id);
                                if (ResourceComment_list != null && ResourceComment_list.id > 0)
                                {
                                    msguser_id = ResourceComment_list.user_id;
                                }
                            }
                            StringBuilder strSql = new StringBuilder();
                            ...
                            if (msguser_id > 0)
                            {
                                strSql.Append("insert into ResourceCommentReply_list(");
                                strSql.Append("ResourceComment_id, user_id, addtime, info_, status, msguser_id)");
                                strSql.Append(" values (");
                                strSql.Append("@ResourceComment_id, @user_id, @addtime, @info_, @status, @msguser_id)");
                                strSql.Append(";select @@IDENTITY");
                                SqlParameter[] parameters_ = {
                        new SqlParameter("@ResourceComment_id", SqlDbType.BigInt),
                          new SqlParameter("@user_id",SqlDbType.Int),
                          new SqlParameter("@addtime",SqlDbType.DateTime),
                          new SqlParameter("@info_",SqlDbType.NVarChar,-1),
                          new SqlParameter("@status",SqlDbType.Int),
                          new SqlParameter("@msguser_id",SqlDbType.Int)
                                    };

                                parameters_[0].Value = reply_id;
                                parameters_[1].Value = user_list.id;
                                DateTime DateTimeNow = DateTime.Now;
                                parameters_[2].Value = DateTimeNow;
                                parameters_[3].Value = Function.HtmlEncode(info);
                                parameters_[4].Value = 0;
                                parameters_[5].Value = msguser_id;

                                parameters = parameters_;
                            }
                            else
                            {
                                strSql.Append("insert into ResourceComment_list(");
                                strSql.Append("Resource_id, user_id, addtime, info_, status, num_dianzan,num_msg)");
                                strSql.Append(" values (");
                                strSql.Append("@Resource_id, @user_id, @addtime, @info_, @status, @num_dianzan, @num_msg)");
                                strSql.Append(";select @@IDENTITY");
                                SqlParameter[] parameters_ = {
                        new SqlParameter("@Resource_id", SqlDbType.BigInt),
                          new SqlParameter("@user_id",SqlDbType.Int),
                          new SqlParameter("@addtime",SqlDbType.DateTime),
                          new SqlParameter("@info_",SqlDbType.NVarChar,-1),
                          new SqlParameter("@status",SqlDbType.Int),
                          new SqlParameter("@num_dianzan",SqlDbType.Int),
                          new SqlParameter("@num_msg",SqlDbType.Int)
                                    };

                                parameters_[0].Value = Resource_list.id;
                                parameters_[1].Value = user_list.id;
                                DateTime DateTimeNow = DateTime.Now;
                                parameters_[2].Value = DateTimeNow;
                                parameters_[3].Value = Function.HtmlEncode(info);
                                parameters_[4].Value = 0;
                                parameters_[5].Value = 0;
                                parameters_[6].Value = 0;
                            ...
                            if (!string.IsNullOrWhiteSpace(sql))
                            {
                                if (msguser_id > 0)
                                {
                                    sql = "INSERT INTO ResourceCommentReplyImg_list (ResourceCommentReply_Id, upload_pic_info, addtime, orderid) VALUES " + sql.Substring(0, sql.Length - 1) + del_img;
                                }
                                else
                                {
                                    sql = "INSERT INTO ResourceCommentImg_list (ResourceComment_Id, upload_pic_info, addtime, orderid) VALUES " + sql.Substring(0, sql.Length - 1) + del_img;
                                }
                            }

                            int addid = ResourceComment_listbll.Add_R_Id_(parameters, strSql, sql);
                            if (addid > 0)
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "您的评论提交操作已完成，在通过审核流程后，该评论会呈现在页面内！" });
                                try
                                {
                                    ...
                                    Body += " <div style=' width: 150px; text-align: justify; color: #888; font-size: 26px; line-height: 1.4; height: 32px;  overflow: hidden; font-weight: lighter;'>评论者：</div>";
                                    ...
                                    Body += "<div  style=' display: flex; justify-content: flex-start;align-items: flex-start; border-bottom: 1px #eee solid; padding-bottom: 30px; margin-bottom: 30px;'>";
                                    Body += " <div style=' width: 150px; text-align: justify; color: #888; font-size: 26px; line-height: 1.4; height: 32px;  overflow: hidden; font-weight: lighter;'>评论日期：</div>";
                                    Body += " <div  style='width: calc(100% - 150px); color: #222;  font-size: 26px; line-height: 1.4; display: flex; justify-content: flex-start; align-items: flex-start;'>" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "</div> ";
                                    Body += " </div>";



                                    Body += "<div  style=' display: flex; justify-content: flex-start;align-items: flex-start; border-bottom: 1px #eee solid; padding-bottom: 30px; margin-bottom: 30px;'>";
                                    Body += " <div style=' width: 150px; text-align: justify; color: #888; font-size: 26px; line-height: 1.4; height: 32px;  overflow: hidden; font-weight: lighter;'>评论内容：</div>";
                                    ...
                                    CommonFunc.ToEmail("温馨提示：收到一条新评论，还望尽快审核处理~", Body, "");
                                }
                                catch (Exception)
                                {
                        ...
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "资源参数异常！" });
                        }
                    }
                    else
                    {
        ...
        public static string GetLiteratureCommentAddFunc(int literatureId, string info)
        {
            ...
            try
            {
                ...
                comment.source_service_log_id = null;
                comment.source_service_log_info_id = null;
                comment.addtime = now;
        ...
        [Obsolete("旧 Workflow/Task 评论入口，最终交付版不再暴露到主流程。")]
        public static string GetWorkflowTaskCommentAddFunc(long GeneratedAssetRecord_id, string info, string[] ImgArr, string task_comment_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    if (!string.IsNullOrWhiteSpace(info))
                    {
                        GeneratedAssetRecord_list GeneratedAssetRecord_list = GeneratedAssetRecord_listbll.SelectSingle("id=" + GeneratedAssetRecord_id + "  and iscos=1 and isshow=1 and istop=1");
                        if (GeneratedAssetRecord_list != null && GeneratedAssetRecord_list.id > 0)
                        {
                            int msguser_id = 0;
                            if (Function.ConvertTo<long>(task_comment_id, 0) > 0)
                            {
                                WorkflowTaskComment_list WorkflowTaskComment_list = WorkflowTaskComment_listbll.SelectSingle("id=" + task_comment_id + " and GeneratedAssetRecord_id=" + GeneratedAssetRecord_id);
                                if (WorkflowTaskComment_list != null && WorkflowTaskComment_list.id > 0)
                                {
                                    msguser_id = WorkflowTaskComment_list.user_id;
                                }
                            }
                            StringBuilder strSql = new StringBuilder();
                            SqlParameter[] parameters = null;
                            if (msguser_id > 0)
                            {
                                strSql.Append("insert into WorkflowTaskReply_list(");
                                strSql.Append("task_comment_id, user_id, addtime, info_, status, msguser_id)");
                                strSql.Append(" values (");
                                strSql.Append("@task_comment_id, @user_id, @addtime, @info_, @status, @msguser_id)");
                                strSql.Append(";select @@IDENTITY");
                                SqlParameter[] parameters_ = {
                        new SqlParameter("@task_comment_id", SqlDbType.BigInt),
                          new SqlParameter("@user_id",SqlDbType.Int),
                          new SqlParameter("@addtime",SqlDbType.DateTime),
                          new SqlParameter("@info_",SqlDbType.NVarChar,-1),
                          new SqlParameter("@status",SqlDbType.Int),
                          new SqlParameter("@msguser_id",SqlDbType.Int)
                                ...
                                parameters_[0].Value = task_comment_id;
                                parameters_[1].Value = user_list.id;
                                DateTime DateTimeNow = DateTime.Now;
                                parameters_[2].Value = DateTimeNow;
                                parameters_[3].Value = Function.HtmlEncode(info);
                            ...
                            {
                                strSql.Append("insert into WorkflowTaskComment_list(");
                                strSql.Append("GeneratedAssetRecord_id, user_id, addtime, info_, status, num_dianzan,num_msg)");
                                strSql.Append(" values (");
                                strSql.Append("@GeneratedAssetRecord_id, @user_id, @addtime, @info_, @status, @num_dianzan, @num_msg)");
                                strSql.Append(";select @@IDENTITY");
                                SqlParameter[] parameters_ = {
                        new SqlParameter("@GeneratedAssetRecord_id", SqlDbType.BigInt),
                          new SqlParameter("@user_id",SqlDbType.Int),
                          new SqlParameter("@addtime",SqlDbType.DateTime),
                          new SqlParameter("@info_",SqlDbType.NVarChar,-1),
                          new SqlParameter("@status",SqlDbType.Int),
                          ...
                          new SqlParameter("@num_msg",SqlDbType.Int)
                                ...
                                parameters_[0].Value = GeneratedAssetRecord_list.id;
                                parameters_[1].Value = user_list.id;
                                DateTime DateTimeNow = DateTime.Now;
                                parameters_[2].Value = DateTimeNow;
                                parameters_[3].Value = Function.HtmlEncode(info);
                            ...
                            if (!string.IsNullOrWhiteSpace(sql))
                            {
                                if (msguser_id > 0)
                                {
                                    sql = "INSERT INTO WorkflowTaskReplyImage_list (WorkflowTaskReply_Id, upload_pic_info, addtime, orderid) VALUES " + sql.Substring(0, sql.Length - 1) + del_img;
                                }
                                else
                                {
                                    sql = "INSERT INTO WorkflowTaskCommentImage_list (WorkflowTaskComment_Id, upload_pic_info, addtime, orderid) VALUES " + sql.Substring(0, sql.Length - 1) + del_img;
                                }
                            }

                            int addid = WorkflowTaskComment_listbll.Add_R_Id_(parameters, strSql, sql);
                            if (addid > 0)
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "您的评论提交操作已完成，在通过审核流程后，该评论会呈现在页面内！" });
                                try
        ...
        [Obsolete("旧 Resource 评论点赞入口，UserCommon.ashx.cs 已移除对应 btn 分发。")]
        public static string GetCommentLikeFunc(long resource_id, string comment_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ...
                    if (Resource_list != null && Resource_list.id > 0)
                    {
                        ResourceComment_list ResourceComment_list = ResourceComment_listbll.SelectSingle("id=" + Function.ConvertTo<int>(comment_id, 0) + " and status=1");
                        if (ResourceComment_list != null && ResourceComment_list.id > 0)
                        {
                            ResourceCommentLike_List ResourceCommentLike_List = ResourceCommentLike_Listbll.SelectSingle("comment_id=" + ResourceComment_list.id + " and user_id=" + user_list.id);
                            if (ResourceCommentLike_List != null && ResourceCommentLike_List.comment_id > 0)
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "已点赞！", ["isCollect"] = true, ["num_dianzan"] = ResourceComment_list.num_dianzan });
                                //取消点赞
                                //string sql = "DELETE FROM ResourceCommentLike_List WHERE resource_id=" + Resource_list.id + " and user_id=" + user_list.id;
                                //sql += "ξLiteratureManagerξUPDATE Resource_list SET num_shoucang=num_shoucang-1 WHERE id=" + Resource_list.id;
                                //if (ResourceCommentLike_Listbll.Sql_D(sql))
                                //{
                                //    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "取消点赞成功！", ["isCollect"] = false });
                                //}
                                //else
                            ...
                            {
                                //点赞
                                string sql = "INSERT INTO ResourceCommentLike_List (comment_id,user_id,addtime) VALUES (" + ResourceComment_list.id + "," + user_list.id + ",GETDATE())";
                                sql += "ξLiteratureManagerξUPDATE ResourceComment_list SET num_dianzan=num_dianzan+1 WHERE id=" + ResourceComment_list.id;
                                if (ResourceCommentLike_Listbll.Sql_D(sql))
                                {
                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "点赞成功！", ["isCollect"] = true, ["num_dianzan"] = ResourceComment_list.num_dianzan + 1 });
                                }
                                else
                                {
                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "点赞失败，请稍后再试！" });
                        ...
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "评论参数异常！" });
                        }
                    }
                    else
                    {
        ...
        [Obsolete("旧 Workflow/Task 点赞入口，最终交付版不再暴露到主流程。")]
        public static string GetWorkflowTaskReactionFunc(long GeneratedAssetRecord_id, string task_comment_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ...
                    if (GeneratedAssetRecord_list != null && GeneratedAssetRecord_list.id > 0)
                    {
                        WorkflowTaskComment_list WorkflowTaskComment_list = WorkflowTaskComment_listbll.SelectSingle("id=" + Function.ConvertTo<int>(task_comment_id, 0) + " and status=1");
                        if (WorkflowTaskComment_list != null && WorkflowTaskComment_list.id > 0)
                        {
                            WorkflowTaskReaction_list WorkflowTaskReaction_list = WorkflowTaskReaction_listbll.SelectSingle("task_comment_id=" + WorkflowTaskComment_list.id + " and user_id=" + user_list.id);
                            if (WorkflowTaskReaction_list != null && WorkflowTaskReaction_list.task_comment_id > 0)
                            {
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "已点赞！", ["isCollect"] = true, ["num_dianzan"] = WorkflowTaskComment_list.num_dianzan });
                                //取消点赞
                                //string sql = "DELETE FROM WorkflowTaskReaction_list WHERE resource_id=" + Resource_list.id + " and user_id=" + user_list.id;
                                //sql += "ξLiteratureManagerξUPDATE Resource_list SET num_shoucang=num_shoucang-1 WHERE id=" + Resource_list.id;
                                //if (WorkflowTaskReaction_listbll.Sql_D(sql))
                                //{
                                //    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "取消点赞成功！", ["isCollect"] = false });
                                //}
                                //else
                            ...
                            {
                                //点赞
                                string sql = "INSERT INTO WorkflowTaskReaction_list (task_comment_id,user_id,addtime) VALUES (" + WorkflowTaskComment_list.id + "," + user_list.id + ",GETDATE())";
                                sql += "ξLiteratureManagerξUPDATE WorkflowTaskComment_list SET num_dianzan=num_dianzan+1 WHERE id=" + WorkflowTaskComment_list.id;
                                if (WorkflowTaskComment_listbll.Sql_D(sql))
                                {
                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "点赞成功！", ["isCollect"] = true, ["num_dianzan"] = WorkflowTaskComment_list.num_dianzan + 1 });
                                }
                                else
                                {
                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "点赞失败，请稍后再试！" });
        ...
        public static string GetModelStatisticsFunc(string startDate, string endDate)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ...
                    DateTime startDate_ = Function.ConvertTo<DateTime>(startDate, DateTime.MinValue);
                    DateTime endDate_ = Function.ConvertTo<DateTime>(endDate, DateTime.MinValue);

                    if (startDate_ <= endDate_ && endDate_ < DateTime.Now)
                    {
                        TimeSpan timeSpan = endDate_.Subtract(startDate_);
                        int time_day = timeSpan.Days + 1;
                        string[] dataArr = new string[time_day];
                        int[] data1_zanArr = new int[time_day];
                        int[] data1_CollectArr = new int[time_day];
                        int[] data1_DaYinArr = new int[time_day];
                        int[] data1_DownArr = new int[time_day];
                        int[] data2_IntegrateConsumeArr = new int[time_day];
                        int[] data2_IntegrateAddArr = new int[time_day];
                        ...
                        {
                            dataArr[i] = startDate_.AddDays(i).ToString("yyyy-MM-dd");


                            DataTable integrateLog_listdt_ = integrateLog_listbll.GetDatatable("select sum(num_integrate) as num from integrateLog_list where DATEDIFF(day,addtime,'" + dataArr[i] + "')=0 and user_id=" + user_list.id + " and num_integrate>0");
                            if (integrateLog_listdt_ != null && integrateLog_listdt_.Rows.Count > 0)
                            {
                                data2_IntegrateAddArr[i] = Function.ConvertTo<int>(integrateLog_listdt_.Rows[0]["num"].ToString(), 0);
                                num_Integrate += data2_IntegrateAddArr[i];
                            }
                            integrateLog_listdt_.Dispose();


                            DataTable integrateLog_listdt = integrateLog_listbll.GetDatatable("select sum(num_integrate) as num from integrateLog_list where DATEDIFF(day,addtime,'" + dataArr[i] + "')=0 and user_id=" + user_list.id + " and num_integrate<0");
                            if (integrateLog_listdt != null && integrateLog_listdt.Rows.Count > 0)
                            {
                                data2_IntegrateConsumeArr[i] = (0 - Function.ConvertTo<int>(integrateLog_listdt.Rows[0]["num"].ToString(), 0));
                                num_Integrate += data2_IntegrateConsumeArr[i];
                            }
                            integrateLog_listdt.Dispose();

                            DataTable ResourceDownloadLog_Listdt_ = ResourceDownloadLog_Listbll.GetDatatable("select count(1) as num from ResourceDownloadLog_List where DATEDIFF(day,addtime, '" + dataArr[i] + "')=0 and type=2 and resource_user_id=" + user_list.id);
                            if (ResourceDownloadLog_Listdt_ != null && ResourceDownloadLog_Listdt_.Rows.Count > 0)
                            {
                                data1_DownArr[i] = Function.ConvertTo<int>(ResourceDownloadLog_Listdt_.Rows[0]["num"].ToString(), 0);
                                num_Down += data1_DownArr[i];
                            }
                            ResourceDownloadLog_Listdt_.Dispose();

                            DataTable ResourceDownloadLog_Listdt = ResourceDownloadLog_Listbll.GetDatatable("select count(1) as num from ResourceDownloadLog_List where DATEDIFF(day,addtime, '" + dataArr[i] + "')=0 and type=1 and resource_user_id=" + user_list.id);
                            if (ResourceDownloadLog_Listdt != null && ResourceDownloadLog_Listdt.Rows.Count > 0)
                            {
                                data1_DaYinArr[i] = Function.ConvertTo<int>(ResourceDownloadLog_Listdt.Rows[0]["num"].ToString(), 0);
                                num_DaYin += data1_DaYinArr[i];
                            }
                            ResourceDownloadLog_Listdt.Dispose();

                            DataTable ResourceCollect_Listdt = ResourceCollect_Listbll.GetDatatable("select count(1) as num from ResourceCollect_List where DATEDIFF(day,addtime,'" + dataArr[i] + "')=0 and resource_user_id=" + user_list.id);
                            if (ResourceCollect_Listdt != null && ResourceCollect_Listdt.Rows.Count > 0)
                            {
                                data1_CollectArr[i] = Function.ConvertTo<int>(ResourceCollect_Listdt.Rows[0]["num"].ToString(), 0);
                                num_Collect += data1_CollectArr[i];
                            }
                            ResourceCollect_Listdt.Dispose();

                            DataTable ResourceLike_Listdt = ResourceLike_Listbll.GetDatatable("select count(1) as num from ResourceLike_List where DATEDIFF(day,addtime,'" + dataArr[i] + "')=0 and resource_user_id=" + user_list.id);
                            if (ResourceLike_Listdt != null && ResourceLike_Listdt.Rows.Count > 0)
                            {
                                data1_zanArr[i] = Function.ConvertTo<int>(ResourceLike_Listdt.Rows[0]["num"].ToString(), 0);
                                num_Zan += data1_zanArr[i];
                            }
                            ResourceLike_Listdt.Dispose();
                        }



                        string[] data3_ResourceArr = new string[0];
                        int[] data3_IntegrateArr = new int[0];
                        int[] data3_LikeArr = new int[0];
                        int[] data3_CollectArr = new int[0];
                        int[] data3_DaYinArr = new int[0];
                        int[] data3_DownArr = new int[0];
                        DataTable ResourceListDt = Resource_listbll.GetDatatable("select id,name from Resource_list where userid=" + user_list.id + " and status=1 order by addtime desc");
                        if (ResourceListDt != null && ResourceListDt.Rows.Count > 0)
                        {
                            data3_ResourceArr = new string[ResourceListDt.Rows.Count];
                            data3_IntegrateArr = new int[ResourceListDt.Rows.Count];
                            data3_LikeArr = new int[ResourceListDt.Rows.Count];
                            data3_CollectArr = new int[ResourceListDt.Rows.Count];
                            data3_DaYinArr = new int[ResourceListDt.Rows.Count];
                            data3_DownArr = new int[ResourceListDt.Rows.Count];
                            for (int i = 0; i < ResourceListDt.Rows.Count; i++)
                            {
                                data3_ResourceArr[i] = Function.HtmlDiscode(ResourceListDt.Rows[i]["name"].ToString());

                                DataTable ResourceDownloadLog_Listdt_ = ResourceDownloadLog_Listbll.GetDatatable("select count(1) as num from ResourceDownloadLog_List where resource_id=" + ResourceListDt.Rows[i]["id"].ToString() + " and addtime>='" + startDate_.ToString("yyyy-MM-dd") + "' and addtime<='" + endDate_.AddDays(1).ToString("yyyy-MM-dd") + "' and type=2 ");
                                if (ResourceDownloadLog_Listdt_ != null && ResourceDownloadLog_Listdt_.Rows.Count > 0)
                                {
                                    data3_DownArr[i] = Function.ConvertTo<int>(ResourceDownloadLog_Listdt_.Rows[0]["num"].ToString(), 0);
                                }
                                ResourceDownloadLog_Listdt_.Dispose();

                                DataTable ResourceDownloadLog_Listdt = ResourceDownloadLog_Listbll.GetDatatable("select count(1) as num from ResourceDownloadLog_List where resource_id=" + ResourceListDt.Rows[i]["id"].ToString() + " and addtime>='" + startDate_.ToString("yyyy-MM-dd") + "' and addtime<='" + endDate_.AddDays(1).ToString("yyyy-MM-dd") + "' and type=1");
                                if (ResourceDownloadLog_Listdt != null && ResourceDownloadLog_Listdt.Rows.Count > 0)
                                {
                                    data3_DaYinArr[i] = Function.ConvertTo<int>(ResourceDownloadLog_Listdt.Rows[0]["num"].ToString(), 0);
                                }
                                ResourceDownloadLog_Listdt.Dispose();

                                DataTable ResourceCollect_Listdt = ResourceCollect_Listbll.GetDatatable("select count(1) as num from ResourceCollect_List where resource_id=" + ResourceListDt.Rows[i]["id"].ToString() + " and addtime>='" + startDate_.ToString("yyyy-MM-dd") + "' and addtime<='" + endDate_.AddDays(1).ToString("yyyy-MM-dd") + "'");
                                if (ResourceCollect_Listdt != null && ResourceCollect_Listdt.Rows.Count > 0)
                                {
                                    data3_CollectArr[i] = Function.ConvertTo<int>(ResourceCollect_Listdt.Rows[0]["num"].ToString(), 0);
                                }
                                ResourceCollect_Listdt.Dispose();

                                DataTable ResourceLike_Listdt = ResourceLike_Listbll.GetDatatable("select count(1) as num from ResourceLike_List where resource_id=" + ResourceListDt.Rows[i]["id"].ToString() + " and addtime>='" + startDate_.ToString("yyyy-MM-dd") + "' and addtime<='" + endDate_.AddDays(1).ToString("yyyy-MM-dd") + "'");
                                if (ResourceLike_Listdt != null && ResourceLike_Listdt.Rows.Count > 0)
                                {
                                    data3_LikeArr[i] = Function.ConvertTo<int>(ResourceLike_Listdt.Rows[0]["num"].ToString(), 0);
                                }
                                ResourceLike_Listdt.Dispose();

                                DataTable integrateLog_listdt = integrateLog_listbll.GetDatatable("select sum(num_integrate) as num from integrateLog_list where addtime>='" + startDate_.ToString("yyyy-MM-dd") + "' and addtime<='" + endDate_.AddDays(1).ToString("yyyy-MM-dd") + "' and type=5 and user_id=" + user_list.id + " and resource_id=" + ResourceListDt.Rows[i]["id"].ToString());
                                if (integrateLog_listdt != null && integrateLog_listdt.Rows.Count > 0)
                                {
                                    data3_IntegrateArr[i] = Function.ConvertTo<int>(integrateLog_listdt.Rows[0]["num"].ToString(), 0);
                                }
                                integrateLog_listdt.Dispose();
                            }
                        }
                        ...
                        str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "搜索时间异常！", ["dataArr"] = dataArr, ["data1_CollectArr"] = data1_CollectArr, ["data1_zanArr"] = data1_zanArr, ["data1_DaYinArr"] = data1_DaYinArr, ["data1_DownArr"] = data1_DownArr, ["data2_IntegrateConsumeArr"] = data2_IntegrateConsumeArr, ["data2_IntegrateAddArr"] = data2_IntegrateAddArr, ["data3_IntegrateArr"] = data3_IntegrateArr, ["data3_LikeArr"] = data3_LikeArr, ["data3_CollectArr"] = data3_CollectArr, ["data3_DaYinArr"] = data3_DaYinArr, ["data3_DownArr"] = data3_DownArr, ["num_Integrate"] = num_Integrate, ["num_Zan"] = num_Zan, ["num_Collect"] = num_Collect, ["num_DaYin"] = num_DaYin, ["num_Down"] = num_Down, ["data3_ResourceArr"] = data3_ResourceArr });
                    }
                    else
                    {
                        str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "搜索时间异常！" });
                    }
                }
                else
                {
        ...
        public static string GetResourceDownloadFunc(int resource_id, int resource_data_id, string type, int user_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0 && user_list.id == user_id)
                {
                    Resource_list Resource_list = Resource_listbll.SelectSingle(" id= " + resource_id + " and status=1 and ResourceClass3_id in(select id from ResourceClass3_List where IsShow=1 and ResourceClass2_Id in(select id from ResourceClass2_List where isshow=1 and ResourceClass1_Id in(select id from ResourceClass1_List where isshow=1)))");
                    if (Resource_list != null && Resource_list.id > 0)
                    {
                        ResourceData_list ResourceData_list = ResourceData_listbll.SelectSingle("id=" + resource_data_id + " and Resource_id=" + Resource_list.id);
                        if (ResourceData_list != null && ResourceData_list.id > 0)
                        {
                            bool isdown_user = false;
                            ResourceDownloadLog_List ResourceDownloadLog_List = ResourceDownloadLog_Listbll.SelectSingle("user_id=" + user_list.id + " and resource_id=" + Resource_list.id + " and resource_data_id=" + ResourceData_list.id);
                            if (ResourceDownloadLog_List != null && ResourceDownloadLog_List.resource_id > 0)
                            {
                                isdown_user = true;
                            }

                            string err = string.Empty;
                            int UserIntegrate = CommonUserFunc.GetUserIntegrateSumFunc(user_list.id, 0);
                            if (Resource_list.points > UserIntegrate)
                            {
                                err = "积分不足！";
                            }
                            if (isdown_user)
                            {
                                if ((type == "1" || type == "4" || type == "5") && CommonFunc.GetCosFileBool(ResourceData_list.up_filename_3mf))
                                {
                        string file_url = fileStorageBaseUrl + ResourceData_list.up_filename_3mf;


                                    string open_url = "" + HttpUtility.UrlEncode(file_url);
                                    if (type == "4")
                                    {
                                        open_url = "" + HttpUtility.UrlEncode(file_url);
                                    }
                                    else if (type == "5")
                                    {
                                        open_url = "" + HttpUtility.UrlEncode(file_url);
                                    }
                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "下载成功！", ["url"] = open_url });
                                }
                                else if (type == "2" && CommonFunc.GetCosFileBool(ResourceData_list.up_filename_3mf))
                                {
                        string file_url = fileStorageBaseUrl + ResourceData_list.up_filename_3mf;

                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "下载成功！", ["url"] = file_url });
                                }
                                else if (type == "3" && CommonFunc.GetCosFileBool(ResourceData_list.up_filename_stl))
                                {
                        string file_url = fileStorageBaseUrl + ResourceData_list.up_filename_stl;

                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "下载成功！", ["url"] = file_url });
                                }
                                else
                                {
                                    str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "请求参数异常！" });
                                }
                            }
                            else
                            {
                                if ((type == "1" || type == "4" || type == "5") && CommonFunc.GetCosFileBool(ResourceData_list.up_filename_3mf))
                                {
                                    bool isdwon = true;
                        string file_url = fileStorageBaseUrl + ResourceData_list.up_filename_3mf;
                                    string sql = "INSERT INTO ResourceDownloadLog_List (addtime, user_id, resource_name, resource_data_name, file_url, file_type, resource_id, resource_data_id,type,resource_user_id) VALUES (GETDATE(), " + user_list.id + ", N'" + Resource_list.name + "', N'" + ResourceData_list.name + "', '" + file_url + "', 'PDF', " + Resource_list.id + ", " + ResourceData_list.id + "," + type + "," + Resource_list.userid + ")";
                                    sql += "ξLiteratureManagerξUPDATE Resource_list SET num_dayin=num_dayin+1 WHERE id=" + Resource_list.id;
                                    sql += "ξLiteratureManagerξUPDATE ResourceData_list SET num_dayin=num_dayin+1 WHERE id=" + ResourceData_list.id;

                                    if (Resource_list.points > 0)
                                    {
                                        if (string.IsNullOrWhiteSpace(err))
                                        {
                                            sql += "ξLiteratureManagerξINSERT INTO integrateLog_list (num_integrate, type, name, info_, addtime, user_id,resource_id,resource_data_id) VALUES (-" + Resource_list.points + ",7,'资源处理扣分','处理资源版本《" + ResourceData_list.name + "》资源成功，扣除 " + Resource_list.points + " 积分！',GETDATE()," + user_list.id + "," + Resource_list.id + "," + ResourceData_list.id + ")";
                                            sql += "ξLiteratureManagerξINSERT INTO integrateLog_list (num_integrate, type, name, info_, addtime, user_id,resource_id,resource_data_id) VALUES (" + Resource_list.points + ",8,'资源被使用得分','文献文件《" + ResourceData_list.name + "》被使用成功，成功获取 " + Resource_list.points + " 积分！',GETDATE()," + Resource_list.userid + "," + Resource_list.id + "," + ResourceData_list.id + ")";
                                        }
                                        else
                                        {
                                            isdwon = false;
                                    ...
                                    if (isdwon)
                                    {
                                        if (ResourceDownloadLog_Listbll.Sql_D(sql))
                                        {
                                            string open_url = "" + HttpUtility.UrlEncode(file_url);
                                            if (type == "4")
                                            {
                                        ...
                                        {
                                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "请求失败，请稍后再试！" });
                                        }
                                    }
                                    else
                                    {
                        ...
                        string file_url = fileStorageBaseUrl + ResourceData_list.up_filename_3mf;
                                    ...
                                    string sql = "INSERT INTO ResourceDownloadLog_List (addtime, user_id, resource_name, resource_data_name, file_url, file_type, resource_id, resource_data_id,type,resource_user_id) VALUES (GETDATE(), " + user_list.id + ", N'" + Resource_list.name + "', N'" + ResourceData_list.name + "', '" + file_url + "', 'PDF', " + Resource_list.id + ", " + ResourceData_list.id + ",2," + Resource_list.userid + ")";
                                    sql += "ξLiteratureManagerξUPDATE Resource_list SET num_xiazai=num_xiazai+1 WHERE id=" + Resource_list.id;
                                    sql += "ξLiteratureManagerξUPDATE ResourceData_list SET num_xiazai=num_xiazai+1 WHERE id=" + ResourceData_list.id;

                                    if (Resource_list.points > 0)
                                    {
                                        if (string.IsNullOrWhiteSpace(err))
                                        {
                                            sql += "ξLiteratureManagerξINSERT INTO integrateLog_list (num_integrate, type, name, info_, addtime, user_id,resource_id,resource_data_id) VALUES (-" + Resource_list.points + ",4,'资源下载扣分','下载文献文件《" + ResourceData_list.name + "》资源成功，扣除 " + Resource_list.points + " 积分！',GETDATE()," + user_list.id + "," + Resource_list.id + "," + ResourceData_list.id + ")";
                                            sql += "ξLiteratureManagerξINSERT INTO integrateLog_list (num_integrate, type, name, info_, addtime, user_id,resource_id,resource_data_id) VALUES (" + Resource_list.points + ",5,'资源被下载得分','文献文件《" + ResourceData_list.name + "》被下载成功，成功获取 " + Resource_list.points + " 积分！',GETDATE()," + Resource_list.userid + "," + Resource_list.id + "," + ResourceData_list.id + ")";
                                        }
                                        else
                                        {
                                            isdwon = false;
                        ...
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "文献文件数据异常！" });
                        }
                    }
                    else
                    {
                        str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "文献数据异常！" });
                    }
                }
                else
                {
        ...
        public static string GetIsDownFunc(int resource_id, int resource_data_id, string type, int user_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
                ...
                if (user_list != null && user_list.id > 0 && user_list.id == user_id)
                {
                    ...
                    if (Resource_list != null && Resource_list.id > 0)
                    {
                        ...
                        string file_url = fileStorageBaseUrl + ResourceData_list.up_filename_stl;
                                ...
                                str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 2, ["info"] = "请求参数异常！" });
                            }
                        }
                        else
                        {
        ...
        public static string GetDrawdownFunc(int ResourceCopy_id)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            string str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "网络繁忙请稍后再试！" });
            try
            {
                ...
                if (user_list != null && user_list.id > 0)
                {
                    ResourceCopy_list ResourceCopy_list = ResourceCopy_listbll.SelectSingle("id=" + ResourceCopy_id + " and userid=" + user_list.id + " and status in(0)");
                    if (ResourceCopy_list != null && ResourceCopy_list.userid > 0)
                    {
                        if (ResourceCopy_listbll.Update("status=-2,addtime=getdate()", "id=" + ResourceCopy_list.id + " and userid=" + user_list.id + " and status in(0)"))
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 1, ["info"] = "回撤审核成功！" });
                        }
                        else
                        {
                            str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "回撤审核失败！" });
                        }
                    }
                    else
                    {
                        str_ = serializer.Serialize(new Dictionary<string, object> { ["status"] = 0, ["info"] = "回撤审核数据异常！" });
                    }
                }
                else
                {
