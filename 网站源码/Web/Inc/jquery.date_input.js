...
DateInput.prototype = {
  build: function() {
    ...
    var nav = $('<div class="jq_input_nav"></div>').append(monthNav, yearNav);
    
    var tableShell = "<table><thead><tr>";
    $(this.adjustDays(this.short_day_names)).each(function() {
      tableShell += "<th>" + this + "</th>";
    });
    tableShell += "</tr></thead><tbody></tbody><thead></thead></table>";
  ...
  selectMonth: function(date) {
    ...
    if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
      ...
      for (var i = 0; i <= numDays; i++) {
        ...
        var taday = new Date(date.getYear(), date.getMonth(), date.getDate(), 12, 00);
      ...
      this.tbody.empty().append(dayCells+"<tr><td colspan=7><span class=jq_table date='" + this.dateToString(date) + "'>今天</span><span class=jq_clear date=''>清空</span></td></tr>");
	  ...
	  $(".jq_table", this.tbody).click(this.bindToObj(function(event) {
        this.changeInput($(event.target).attr("date"));
      }));
	  
	  $(".jq_clear", this.tbody).click(this.bindToObj(function(event) {
        this.changeInput($(event.target).attr("date"));
      }));
