...
(function (k) {
	var E, F, G, H, I, J, r = {},
		...
		u = [function (b) {
			var a = e.event;
			b.treeObj.unbind(a.NODECREATED)
				.unbind(a.CLICK)
				.unbind(a.EXPAND)
				.unbind(a.COLLAPSE)
				.unbind(a.ASYNC_SUCCESS)
		...
		g = {
			addNodeCache: function (b, a) {
				g.getCache(b)
					.nodes[g.getNodeCacheId(a.tId)] = a
			},
			...
			addCreatedNode: function (b, a) {
				(b.callback.onNodeCreated || b.view.addDiyDom) && g.getRoot(b)
					.createdNodes.push(a)
			},
			...
			getNodeName: function (b, a) {
				console.log('a[b.data.key.name]', a[b.data.key.name])
				return "" + a[b.data.key.name]
			},
			getNodeTitle: function (b, a) {
				console.log('a[b.data.key.title === "" ? b.data.key.name : b.data.key.title]', ("" + a[b.data.key.title === "" ? b.data.key.name : b.data.key.title]))
			...
			getNodesByFilter: function (b, a, c, d, f) {
				if (!a) return d ? null : [];
				for (var h = b.data.key.children, l = d ? null : [], e = 0, i = a.length; e < i; e++) {
			...
			removeNodeCache: function (b, a) {
				...
				g.getCache(b)
					.nodes[g.getNodeCacheId(a.tId)] = null
			},
		...
		m = {
			...
			unbindTree: function (b) {
				b.treeObj.unbind("click", m.proxy)
					.unbind("dblclick", m.proxy)
					.unbind("mouseover", m.proxy)
					.unbind("mouseout", m.proxy)
					.unbind("mousedown", m.proxy)
					.unbind("mouseup", m.proxy)
		...
		i = {
			addNodes: function (b, a, c, d) {
				if (!b.data.keep.leaf || !a || a.isParent)
					if (j.isArray(c) || (c = [c]), b.data.simpleData.enable && (c = g.transformTozTreeFormat(b, c)), a) {
			...
			asyncNode: function (b, a, c, d) {
				...
				else if (j.apply(b.callback.beforeAsync,
					[b.treeId, a], !0) == !1) return j.apply(d), !1;
				if (a) a.isAjaxing = !0, k("#" + a.tId + e.id.ICON)
					.attr({
						style: "",
				...
				for (f = 0, h = b.async.autoParam.length; a && f < h; f++) {
					...
					l ? (n = typeof a[m] == "string" ? '"' : "", p += '"' + o + ('":' + n + a[m])
						.replace(/'/g, "\\'") + n + ",") : p += o + ("=" + a[m])
							.replace(/&/g, "%26") + "&"
				}
				if (j.isArray(b.async.otherParam))
					for (f = 0, h = b.async.otherParam.length; f <
						h; f += 2) l ? (n = typeof b.async.otherParam[f + 1] == "string" ? '"' : "", p += '"' + b.async.otherParam[f] + ('":' + n + b.async.otherParam[f + 1])
							.replace(/'/g, "\\'") + n + ",") : p += b.async.otherParam[f] + ("=" + b.async.otherParam[f + 1])
								.replace(/&/g, "%26") + "&";
				else
					for (var r in b.async.otherParam) l ? (n = typeof b.async.otherParam[r] == "string" ? '"' : "", p += '"' + r + ('":' + n + b.async.otherParam[r])
						.replace(/'/g, "\\'") + n + ",") : p += r + ("=" + b.async.otherParam[r])
							.replace(/&/g, "%26") + "&";
			...
			cancelPreSelectedNode: function (b, a) {
				for (var c = g.getRoot(b)
					.curSelectedList, d = c.length - 1; d >= 0; d--)
					if (!a || a === c[d])
						if (k("#" + c[d].tId + e.id.A)
							.removeClass(e.node.CURSELECTED), a) {
			...
			expandCollapseNode: function (b, a, c, d, f) {
				...
				if (a) {
					...
					if (!a.open && a.isParent && (!k("#" + a.tId + e.id.UL)
						.get(0) || a[l] && a[l].length > 0 && !k("#" + a[l][0].tId)
			...
			makeDOMNodeIcon: function (b, a, c) {
				var d = g.getNodeName(a, c),
					//d = a.view.nameIsHTML ? d : d.replace(/&/g, "&amp;")
					//	.replace(/</g, "&lt;")
					//	.replace(/>/g, "&gt;");
					d = a.view.nameIsHTML ? d : d.replace(/&/g, "&amp;")
			...
			removeChildNodes: function (b, a) {
				if (a) {
					...
					if (d) {
						...
						delete a[c];
						b.data.keep.parent ? k("#" + a.tId + e.id.UL)
							.empty() :
							(a.isParent = !1, a.open = !1, c = k("#" + a.tId + e.id.SWITCH), d = k("#" + a.tId + e.id.ICON), i.replaceSwitchClass(a, c, e.folder.DOCU), i.replaceIcoClass(a, d, e.folder.DOCU), k("#" + a.tId + e.id.UL)
								.remove())
					}
				}
			},
			...
			setNodeName: function (b, a) {
				...
				j.apply(b.view.showTitle, [b.treeId, a], b.view.showTitle) && k("#" + a.tId + e.id.A)
					.attr("title", !c ? "" : c)
			},
			setNodeTarget: function (b) {
				k("#" + b.tId + e.id.A)
					.attr("target", i.makeNodeTarget(b))
			},
		...
		init: function (b, a, c) {
			...
			c = {
				...
				getSelectedNodes: function () {
					for (var a = [], b = g.getRoot(this.setting)
						.curSelectedList, c = 0, d = b.length; c < d; c++) a.push(b[c]);
					return a
				},
				...
				reAsyncChildNodes: function (a, b, c) {
					if (this.setting.async.enable) {
						...
						if (b == "refresh") {
							...
							a[b] = [];
							j ? this.setting.treeObj.empty() : k("#" + a.tId + e.id.UL)
								.empty()
						}
				...
				updateNode: function (a) {
					a && k("#" + a.tId)
						.get(0) && j.uCanDo(this.setting) && (i.setNodeName(this.setting, a), i.setNodeTarget(a), i.setNodeUrl(this.setting, a), i.setNodeLineIcos(this.setting, a), i.setNodeFontCss(this.setting, a))
				}
			};
			b.treeTools =
				c;
