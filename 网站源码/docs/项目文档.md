...
| 项目项 | 内容 |
| --- | --- |
| 项目类型 | ASP.NET Web Forms + C# + SQL Server 文献管理系统 |
| 主项目源码目录 | `网站源码` |
| RAG 模块源码目录 | `RAG` |
| 数据库 | `manage_db_final` |
| 文档日期 | 2026 年 7 月 1 日 |
...
- [2. 系统需求分析](#2-系统需求分析)
- [3. 系统总体架构](#3-系统总体架构)
- [4. 技术栈与运行环境](#4-技术栈与运行环境)
- [5. 系统功能模块](#5-系统功能模块)
- [6. 数据库设计](#6-数据库设计)
- [7. 核心业务流程](#7-核心业务流程)
- [8. 权限控制与安全机制](#8-权限控制与安全机制)
- [9. 文件上传、存储与下载机制](#9-文件上传存储与下载机制)
- [10. 管理员后台功能](#10-管理员后台功能)
- [11. 统计报表与数据查询](#11-统计报表与数据查询)
- [12. 项目亮点](#12-项目亮点)
- [13. 关键代码文件说明](#13-关键代码文件说明)
- [14. 系统运行与部署说明](#14-系统运行与部署说明)
- [15. 项目总结](#15-项目总结)
- [附录 A. 数据库全表结构](#附录-a-数据库全表结构)
...
系统将 PDF 保存在服务器目录中（或使用腾讯云COS），数据库保存相对路径、文件名、大小、类型和状态。下载时根据文献状态、登录用户、积分或免费下载权益判断是否允许下载，并记录 `LiteratureDownloadLog`。批量下载会检查文件数量、总大小和路径合法性，确保文件路径位于上传根目录内，降低路径穿越风险。
...
普通用户登录状态由 `user_id`、`user_tel`、`user_code` Cookie 和数据库 `user_list.code` 共同确认。未登录用户可以浏览公开内容，但不能上传、评论、点赞、收藏、下载受控 PDF 或提交申诉。相关页的`Page_Load` 中会校验登录，未登录则跳转或提示。

### 8.2 管理员权限

后台管理员通过 `Web/admin/Login.aspx.cs` 登录，管理员信息存储在 `admin` 表。后台页面通过登录校验函数判断当前管理员是否有效，并结合 `popedom`、`popedomhead` 等菜单权限表控制可访问功能。管理员账号支持锁定状态，登录日志记录在 `admin` 或登录相关日志表中。

### 8.3 SQL 注入防护、XSS 与输出编码

项目中核心逻辑已使用 `SqlParameter`，例如事务、RAG 索引触发和部分服务类，方式用户的恶意SQL注入，提升系统稳定性与安全性。
对标题、摘要、评论、机构、反馈内容等用户可输入字段，统一使用 HTML 编码输出，避免 XSS 风险。


### 8.4 上传安全

上传限制由 `UploadPolicy` 和 `Web.config` 共同控制，包括单个 PDF、图片、附件、批量数量和总大小。`UserCommon.ashx.cs` 限制 JSON 请求体大小，并对需要登录的操作进行用户校验。
...
系统将文件本体保存在服务器目录，将文件元数据写入数据库。`LiteratureFile` 保存文献 PDF 和附件的 `file_name`、`file_path`、`file_size`、`mime_type`、`file_type` 和 `statuS',从而避免数据库存储大二进制文件，提高备份和迁移灵活性。
...
首页可统计公开文献数量、作者数量、机构数量、近 12 个月上传或发布趋势等信息。统计通常从 `Literature`、`Author`、`Institution`、`LiteratureDownloadLog` 等表聚合得出。

### 11.2 文献列表统计

文献搜索页和分类页可按分类、年份、来源、关键词等条件筛选公开文献，并显示分页结果。列表统计依赖 `status=1` 和 `canonical_literature_id is null`，保证只统计正式公开主文献。

### 11.3 文献详情统计

详情页可展示评论数、点赞数、收藏数、下载状态和相关文献信息。点赞和收藏从 `LiteratureLike`、`LiteratureFavorite` 统计，下载从 `LiteratureDownloadLog` 统计。

### 11.4 用户中心统计

用户中心可统计个人投稿总数、待审核数、已通过数、已驳回数和通知数量，数据主要来自 `Literature`、`NoticeLog_List`、`ServiceLog_List` 等表。
...
项目覆盖文献上传、PDF 解析、审核、公开检索、详情展示、下载、评论、点赞收藏、作者机构维护、后台导入导出和统计查询，形成了从数据进入系统到公开展示和后续维护的完整业务闭环。

### 12.2 数据库关系复杂度适中

系统围绕 `Literature` 建立作者、机构、期刊、会议、标签、附件、评论、下载、点赞、收藏等关系。尤其是作者-文献多对多关系、文献-作者-机构三元关系和作者机构历史表，能够体现数据库课程设计中的关系建模能力。

### 12.3 作者消歧设计较保守

系统没有简单按姓名合并作者，而是优先使用明确 `author_id`，没有 ID 时再结合姓名、当前机构、历史机构和论文机构进行判断。无法确认时创建新作者并保留后台合并入口，降低同名作者误合并风险。

### 12.4 机构历史管理

系统通过 `AuthorInstitutionHistory` 和 `LiteratureAuthorInstitutionMap` 同时记录作者历史机构和某篇论文中的具体机构，使作者机构信息能随发表时间变化而展示。
...
系统使用 `status=-1` 进行逻辑删除，避免误删文献、机构和其他主数据，同时支持定时删除无用数据。登录日志、状态日志、导入错误、导出日志和作者合并日志增强了系统可追溯性。
...
`BLLBase<T>`、`DALCommon<T>` 和 `DBHelper` 提供了通用增删改查、分页、实体转换和数据库执行能力，使大量页面可以复用基础数据访问逻辑。

### 12.9 RAG 与图谱扩展体现系统集成能力

RAG 服务将 SQL Server 文献主数据与 Chroma 向量库、m3e embedding、DeepSeek 生成能力结合；关系图谱则利用 SQL Server 关系表输出节点和边并由 vis-network 展示，体现了传统 Web 系统与智能检索、可视化模块的集成。
...
### A.16 dbo.integrateExchangeLog_list
...
### A.18 dbo.integrateLogType_list
...
### A.48 dbo.ServiceLogStatus_List
