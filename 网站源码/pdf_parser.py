...
ABSTRACT_COLLECT_CHAR_LIMIT = 8000
...
def looks_like_heading(line: str) -> bool:
    lower = normalize_line(line).lower().strip(" .:-")
    if not lower:
        return False
    if lower in {"abstract", "摘要", "keywords", "keyword", "index terms", "introduction"}:
        return True
    if re.match(r"^(?:\d+(?:\.\d+)*\.?\s+|[ivx]+\.?\s+)?(?:keywords?|index terms|introduction)\b", lower):
        return True
    if re.match(r"^(?:doi|arxiv)\b", lower):
        return True
    return False
...
def reliable_author_candidates_from_line(line: str, title_words: Optional[Set[str]] = None) -> List[str]:
    candidates: List[str] = []
    for author in split_candidate_authors(line):
        cleaned = clean_extracted_field(author, 180)
        if is_reliable_author_candidate(cleaned, title_words):
            candidates.append(cleaned)
    return clean_extracted_list(candidates, MAX_AUTHOR_COUNT)


def looks_like_author_line(line: str) -> bool:
    parsed = reliable_author_candidates_from_line(line)
    if not parsed:
        return False
    lower = (line or "").lower()
    ...
    for line in lines[max(0, start_index):18]:
        ...
        if contains_affiliation_hint(line):
            if candidates:
                break
            continue
        parsed = reliable_author_candidates_from_line(line, title_words)
...
def extract_keywords(text: str) -> List[str]:
    ...
    for pattern in patterns:
        ...
        keyword_line = re.sub(r"^[\s:：\-–—−?]+", "", keyword_line)
        keyword_line = re.split(r"\s{2,}|\babstract\b|摘要|\bintroduction\b", keyword_line, maxsplit=1, flags=re.IGNORECASE)[0]
        keywords = [
            normalize_line(re.sub(r"^[\s:：\-–—−?]+|[\s:：\-–—−?]+$", "", item))
            for item in re.split(r"[,;，；]", keyword_line)
            if normalize_line(re.sub(r"^[\s:：\-–—−?]+|[\s:：\-–—−?]+$", "", item))
        ]
...
def extract_abstract(text: str, lines: Sequence[str], title: str, authors: Sequence[str]) -> str:
    patterns = [
        r"(?im)^\s*abstract\b\s*[:：—\-]?\s*(.*?)(?=^\s*(?:keywords?|index terms|(?:\d+(?:\.\d+)*\.?\s+|[ivx]+\.?\s+)?introduction)\b)",
        r"(?im)^\s*摘要\s*[:：—\-]?\s*(.*?)(?=^\s*(?:关键词|關鍵詞|(?:\d+(?:\.\d+)*\.?\s+)?引言))",
    ]
    for pattern in patterns:
    ...
    for line in lines:
        ...
        if collecting:
            ...
            if len(clean_paragraph(" ".join(parts))) >= ABSTRACT_COLLECT_CHAR_LIMIT:
                break
...
ABSTRACT_TAIL_NOISE_PATTERNS = [
    r"\s+(?:Index\s*Terms?|Keywords?)\s*[—–\-:：?].*$",
    r"\s+#\s*Project\s+lead\b.*$",
    r"\s+\*+\s*Core\s+technical\s+contribution\b.*$",
    r"\s+This\s+work\s+is\s+licensed\s+under\b.*$",
    r"\s+This\s+article\s+is\s+licensed\s+under\b.*$",
    r"\s+This\s+work\s+is\s+available\s+under\b.*$",
    r"\s+https?://doi\.org/\S+.*$",
    r"\s+doi\s*[:：]\s*10\.\S+.*$",
    r"\s+(?:©|Copyright)\b.*$",
    r"\s+All\s+rights\s+reserved\b.*$",
    r"\s+SIGGRAPH\s+Conference\s+Papers\b.*$",
    r"\s+ACM\s+Reference\s+Format\b.*$",
]


def strip_abstract_tail_noise(text: str) -> str:
    cleaned = text or ""
    previous = None
    while previous != cleaned:
        previous = cleaned
        for pattern in ABSTRACT_TAIL_NOISE_PATTERNS:
            cleaned = re.sub(pattern, "", cleaned, flags=re.IGNORECASE | re.DOTALL)
        cleaned = re.sub(r"\s+(?:Preprint|Accepted\s+manuscript|Manuscript)\.?\s*$", "", cleaned, flags=re.IGNORECASE)
        cleaned = cleaned.strip(" ,;:-")
    return cleaned


def clean_abstract_text(text: str) -> str:
    text = clean_extracted_field(text, ABSTRACT_MAX_LENGTH)
    if not text:
        return ""
    text = strip_abstract_tail_noise(text)
    ...
    text = re.sub(r"\s+", " ", text)
    text = repair_common_pdf_word_splits(text).strip(" ,;:-")
    return strip_abstract_tail_noise(text)


PUBLISHER_PATTERNS = [
    (r"\bIEEE\b|Institute of Electrical and Electronics Engineers", "IEEE"),
    (r"\bACM\b|Association for Computing Machinery", "ACM"),
    (r"\bSpringer\b|Springer Nature", "Springer"),
    (r"\bElsevier\b|ScienceDirect", "Elsevier"),
    (r"\bWiley\b", "Wiley"),
    (r"\bMDPI\b", "MDPI"),
    (r"\bNature Portfolio\b|nature\.com", "Nature Portfolio"),
    (r"\bPLOS\b|Public Library of Science", "PLOS"),
    (r"\bOxford University Press\b", "Oxford University Press"),
    (r"\bCambridge University Press\b", "Cambridge University Press"),
]
...
def extract_publication_pages(source: str) -> str:
    match = re.search(
        r"\bpp?\.?\s*([A-Za-z]?\d{1,5}\s*[-–—]\s*[A-Za-z]?\d{1,5})\b",
        source or "",
        re.IGNORECASE,
    )
    if not match:
        return ""
    pages = re.sub(r"\s*[-–—]\s*", "-", match.group(1))
    return clean_short_bibliographic_value(pages, 60)
...
def extract_bibliographic_info(lines: Sequence[str], metadata: Dict, page_count: int, doi: str) -> Dict[str, str]:
    ...
    if looks_like_journal_header(source) and "ieee" in source.lower():
        ...
        publisher = infer_publisher(source, doi)

    pages = extract_publication_pages(source)

    return {
        "volume": clean_short_bibliographic_value(volume, 60),
        "issue": clean_short_bibliographic_value(issue, 60),
        "pages": pages,
        "publisher": clean_extracted_field(publisher, 120),
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            return {
                ...
                "abstract": clean_extracted_field(abstract, ABSTRACT_MAX_LENGTH),
